...
module.exports = function* ( db, config ) {
	...
	if ( !Array.isArray( config.collections ) ) {
		...
		while ( ++i < l ) {
			collection                      = k[i];
			collections[collection]         = new TableOrSProc( config.collections[collection], db, config.__connection__ );
		...
		while ( ++i < l ) {
			...
			collections[collection]         = new TableOrSProc( collection, db, config.__connection__ );
			collections[collection].methods = accessibleMethods( collection, config.access, DEFAULTS.access );
		}
	}
